...
<body class="bg-gray-100">
  ...
  <div id="sidebar" class="bg-gradient-to-b from-gray-900 to-gray-800 text-white w-72 space-y-8 py-8 px-4 fixed inset-y-0 left-0 transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out shadow-2xl">
    ...
    <nav class="space-y-4">
      <a href="" class="flex items-center space-x-4 px-6 py-3 rounded-lg hover:bg-gray-700 transition-colors {{ request()->routeIs('admin.index') ? 'bg-blue-600' : '' }}">
        <i class="fas fa-tachometer-alt text-2xl"></i>
        <span class="text-xl">Dashboard</span>
      </a>
      <a href="" class="flex items-center space-x-4 px-6 py-3 rounded-lg hover:bg-gray-700 transition-colors {{ request()->routeIs('admin.user.index') ? 'bg-blue-600' : '' }}">
        <i class="fas fa-users text-2xl"></i>
        <span class="text-xl">User</span>
      </a>
      <a href="" class="flex items-center space-x-4 px-6 py-3 rounded-lg hover:bg-gray-700 transition-colors {{ request()->routeIs('admin.gudang.index') ? 'bg-blue-600' : '' }}">
        <i class="fas fa-warehouse text-2xl"></i>
        <span class="text-xl">Gudang</span>
      </a>
      <a href="" class="flex items-center space-x-4 px-6 py-3 rounded-lg hover:bg-gray-700 transition-colors {{ request()->routeIs('admin.kategori.index') ? 'bg-blue-600' : '' }}">
        <i class="fas fa-list text-2xl"></i>
        <span class="text-xl">Kategori</span>
      </a>
      <a href="" class="flex items-center space-x-4 px-6 py-3 rounded-lg hover:bg-gray-700 transition-colors {{ request()->routeIs('admin.produk.index') ? 'bg-blue-600' : '' }}">
